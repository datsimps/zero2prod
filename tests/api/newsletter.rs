...
async fn invalid_password_is_rejected() {
    ...
    // Real username random/wrong password
    let username = &app.test_user.username;
    let pass = Uuid::new_v4().to_string();

    println!("username: {}, password: {}", &app.test_user.username, &app.test_user.password);
    println!("password fake: {}", &pass);
    // make sure theyre not equal
    assert_ne!(app.test_user.password, pass);
